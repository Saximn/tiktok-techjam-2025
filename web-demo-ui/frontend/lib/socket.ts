...
export class SocketManager {
  ...
  // use this for mac machine because airplay will use port 5000
  constructor(serverUrl = "http://localhost:5002") {
    this.serverUrl = serverUrl;
    this.socket = io(serverUrl);
  }
  ...
  checkRoom(
    roomId: string
  ): Promise<{ exists: boolean; viewerCount?: number }> {
    return new Promise((resolve) => {
  ...
  startAudioProcessing(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.once("audio_processing_error", (error) =>
        reject(error.error)
      );
    });
  }
  ...
  onAudioProcessingStarted(
    callback: (data: { room_id: string; status: string }) => void
  ) {
    this.socket.on("audio_processing_started", callback);
  }

  onAudioProcessingStopped(
    callback: (data: { room_id: string; status: string }) => void
  ) {
  ...
  onOffer(
    callback: (data: { offer: RTCSessionDescriptionInit; from: string }) => void
  ) {
  ...
  onIceCandidate(
    callback: (data: { candidate: RTCIceCandidateInit; from: string }) => void
  ) {
    this.socket.on("ice_candidate", callback);
  }

  onViewerJoined(
    callback: (data: { userId: string; viewerCount?: number }) => void
  ) {
